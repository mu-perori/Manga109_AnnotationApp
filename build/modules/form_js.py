...
def make_js(input_path, output_path, manga_title):
		...
		with open(output_path + "/" + manga_title + "/coordinates.js", 'w') as f:
			output_text = 'const coordinateData = ' + str(output_dict).replace('\'title\'', 'title').replace('\'characters\'', 'characters').replace('\'characterId\'', 'characterId').replace('\'name\'', 'name').replace('\'index\'', 'index').replace('\'width\'', 'width').replace('\'height\'', 'height').replace('\'frame\'', 'frame').replace('\'dialogue\'', 'dialogue').replace('\'text\'', 'text').replace('\'face\'', 'face').replace('\'characterBody\'', 'characterBody').replace('\'pages\'', 'pages').replace('\'id\'', 'id').replace('\'xmin\'', 'xmin').replace('\'ymin\'', 'ymin').replace('\'xmax\'', 'xmax').replace('\'ymax\'', 'ymax')
			f.write(output_text)
